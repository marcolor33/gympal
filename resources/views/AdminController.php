...
SELECT coach_id,GROUP_CONCAT(subject_name SEPARATOR ',') as subjects, GROUP_CONCAT(distinct category_name SEPARATOR ',') as categories ,GROUP_CONCAT(distinct category_id SEPARATOR ',') as category_id,GROUP_CONCAT(K.subject_id SEPARATOR ',') as subject_id FROM (SELECT C.*,T.subject_name,T.category_name, T.category_id FROM `coachAvailableSubject` C left Join (SELECT S.name_chin as subject_name,S.subject_id,S.category_id, Z.name_chin as category_name FROM `subject` S left join `category` Z on Z.category_id = S.category_id) T on T.subject_id = C.subject_id) K GROUP BY coach_id
  ...
  where C.account_id in (SELECT T.coach_id FROM `topCoach` T )");
        ...
        $counter = DB::table('case')->where('status','=','closed')->count();
        ...
        return view('home',['coachs'=>$coachs,'cases'=>$cases,'categories'=>$categories,'categoriesTable'=>$categoriesTable,'hkDistricts'=>$hkDistricts,'knDistricts'=>$knDistricts,'ntDistricts'=>$ntDistricts,'otherDistricts'=>$otherDistricts,'extend'=>$extend, 'actionName'=>$actionName,'counter'=>$counter]);
...
order by T.order asc');
    ...
    public function list_student()
    {
        ...
        if (isset($_SESSION['user'])) {
            ...
            {
                $students = DB::table('account')->where('account.account_type', '=', 'student')->get();
...
SELECT coach_id,GROUP_CONCAT(subject_name SEPARATOR ',') as subjects, GROUP_CONCAT(distinct category_name SEPARATOR ',') as categories ,GROUP_CONCAT(distinct category_id SEPARATOR ',') as category_id,GROUP_CONCAT(K.subject_id SEPARATOR ',') as subject_id FROM (SELECT C.*,T.subject_name,T.category_name, T.category_id FROM `coachAvailableSubject` C left Join (SELECT S.name_chin as subject_name,S.subject_id,S.category_id, Z.name_chin as category_name FROM `subject` S left join `category` Z on Z.category_id = S.category_id) T on T.subject_id = C.subject_id) K GROUP BY coach_id
    ...
    public function public_list_case()
    {
	   ...
	   $cases = DB::table('case')->leftJoin('subject','case.subject_id','=','subject.subject_id')->leftJoin('district','case.district_id','=','district.district_id')->leftjoin('category','category.category_id','=','subject.category_id')->select('case.*','subject.name_chin as subject_name','category.name_chin as category_name','category.category_id as category_id' ,'district.name_chin as district_name')->orderBy('time_created','desc')->get();
        ...
        if (isset($_SESSION['user'])) {
                ...
                
            {
               $extend = 'masters.admin_master';
                ...
                return view('view_case',['case'=>$case,'hkDistricts'=>$hkDistricts,'knDistricts'=>$knDistricts,'ntDistricts'=>$ntDistricts,'otherDistricts'=>$otherDistricts,'categoriesTable'=>$categoriesTable,'categories'=>$categories, 'coachs'=>$coachs,'extend'=>$extend,'interest'=>$interest]);
            }
...
//        echo $request;
    ...
    public function edit_case(Request $request)
    {
        ...
        $rules = array(
             ...
             'special_requirement'=>'regex:/[a-zA-Z0-9\p{L}_ -,\/]*$/'
        );
        $validator = Validator::make($request->all(), $rules);
         if ($validator->fails()) {
            return response()->json(['success'=>false, 'errors'=>$validator->errors()],422);
    ...
    public function edit_student_profile(Request $request)
    {
        ...
        $this->validate($request,
            [
                ...
                'chinName' => 'regex:/[p{L}]*$/',
                'HKID' => 'required|regex:/[a-zA-Z0-9()]*$/',
                'address' => 'required|regex:/[a-zA-Z0-9\p{L}_ -,\/]*$/',
                'district' => 'required|regex:/[0-9]*$/',
        ...
        public function edit_coach_cv(Request $request)
    {
        ...
        $this->validate($request,
            [
                'coachIntroduction' => 'required|regex:/[a-zA-Z0-9\p{L}_ -,.\/]*$/',
...
//                                  'path'=>$path.$filename,
        ...
        public function list_student_interest()
    {
        ...
        $interests = DB::table('studentInterested')->leftJoin('account','account.account_id','=','studentInterested.coach_id')->select('studentInterested.*','account.account_code as coach_code','account.email as coach_email')->leftJoin('account','account.account_id','=','studentInterested.student_id')->select('studentInterested.*','account.account_code as student_code','account.email as student_email')->get();
