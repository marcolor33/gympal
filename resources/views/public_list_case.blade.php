...
<table class="stripe" id="mytable">
    ...
    <thead><tr><th>個案日期</th><th>個案編號</th><th>學生性別</th><th>教練性別</th><th>運動類別</th><th>運動編號</th><th>上堂地區</th><th>地區編號</th><th>上堂場地</th><th>上堂模式</th><th>上堂次數</th><th>每堂學費</th><th>類別編號</th><th>類別名稱</th><th>詳細</th></tr></thead>
...
//        alert($('[name="search"]').val() == '');
    ...
    function prepare_data(){
        ...
        $('#mytable_body').append('<tr><td>'+time_created+'</td><td>'+case_id+'</td><td>'+student_sex+'</td><td>'+coach_sex+'</td><td>'+subject_name+'</td><td>'+subject_id+'</td><td>'+district_name+'</td><td>'+district_id+'</td><td>'+venue+'</td><td>'+mode+'</td><td>'+frequency+'</td><td>$'+fee+'</td><td>'+category_id+'</td><td>'+category_name+'</td><td>'+action+'</td></tr>');
    ...
    if ($('[name="student_sex"]').val() != 'default')
        {
            ...
            if ($('[name="student_sex"] :selected').text() != data[2])
                {
                   return false; 
                }
        }
      ...
      if ($('[name="coach_sex"]').val() != 'default')
        {
            ...
            if ($('[name="coach_sex"] :selected').text() != data[3])
                {
                    return false;
                }
        }
        ...
        {
            if ($('[name="mode"] :selected').text() != data[9])
                {
                   return false;
                }
        }
        ...
        {
            if ($('[name="venue"] :selected').text()!= data[8])
                {
                    return false;
                }
        }
        ...
        {
            if ( $('[name="category"] :selected').val() != data[12])
                {
                    return false;
                }
        }
...
//                    console.log((parseInt($('[name="min_pay"]').val()) > parseInt(data[11].replace("$",""))) + "data: "+parseInt(data[11].replace("$","")) + "minpay : " + parseInt($('[name="min_pay"]').val()));
                    return false;
...
//                    console.log((parseInt($('[name="max_pay"]')) > parseInt(data[11].replace("$",""))) + "data: "+parseInt(data[11].replace("$","")) );
                    return false;
                }
